...
def build_prompt(config, raw_stack, source_info):
    ...
    prompt.append('- 【重要】hunk 的行号必须准确计算。请仔细数清楚完整文件中的行号，确保 @@ -X,Y +X,Y @@ 中的 X 值正确。如果不确定行号，请返回 NO_SAFE_PATCH')
    prompt.append('')
    ...
    prompt.append('')
    prompt.append('【重要】补丁中的文件路径必须使用上面给出的 repo 相对路径，不要省略任何目录层级。')
    prompt.append('')
    prompt.append('返回以下格式之一:')
    prompt.append('')
    prompt.append('### 选项 1: unified diff')
    prompt.append('```diff')
    prompt.append('--- a/%s' % source_info.get('repo_relative_path', 'path/to/File.java').replace('\\', '/'))
    prompt.append('+++ b/%s' % source_info.get('repo_relative_path', 'path/to/File.java').replace('\\', '/'))
